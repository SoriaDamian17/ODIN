...
<div class="panel" id="conditions-panel" style="display:none;">
	...
	<div class="form-group">
		<label for="id_product" class="control-label col-lg-3">{l s='Productos'}</label>
		<div class="col-lg-9">
			<div class="col-lg-8">
			...
			<div class="col-lg-4">				
					<select class="id_product_value" style="display:none;" id="id_product_">
						{foreach from=$products item='product'}
							<option value="{$product.id_product}">{$product.name}</option>
						{/foreach}
					</select>				
			...
			<div class="col-lg-1">
				<a class="btn btn-default" href="#" id="add_condition_product">
	...
	<div class="form-group">
		<label for="id_customer" class="control-label col-lg-3">{l s='Clientes'}</label>
		<div class="col-lg-9">
			<div class="col-lg-8">
				<select id="id_customer">
					{foreach from=$customers item='customer'}
						<option value="{$customer.id_customer}">{$customer.lastname} {$customer.firstname} - {$customer.email}</option>
					{/foreach}
				</select>
			</div>
			<div class="col-lg-4">				
					<select class="id_customer_value" style="display:none;" id="id_customer_">
						{foreach from=$customers item='customer'}
							<option value="{$customer.id_customer}">{$customer.lastname} {$customer.firstname} - {$customer.email}</option>
						{/foreach}
					</select>				
			...
			<div class="col-lg-1">
				<a class="btn btn-default" href="#" id="add_condition_customer">
					<i class="icon-plus-sign"></i> {l s='Add condition'}
				</a>
			</div>
		</div>
...
$(document).ready(function() {
	...
	$('#add_condition_customer').click(function() {
		var id_condition = add_condition(current_id_condition_group, 'customer', $('#id_customer option:selected').val());
		if (!id_condition)
			return false;
		
		
		var html = '<tr id="'+id_condition+'"><td>{l s='Customer'}</td><td>'+$('#id_customer option:selected').html()+'</td><td><a href="#" onclick="delete_condition(\''+id_condition+'\');" class="btn btn-default"><i class="icon-remove"></i> {l s='Delete'}</a></td></tr>';
		appendConditionToGroup(html);
	...
	$('#add_condition_product').click(function() {
		var id_condition = add_condition(current_id_condition_group, 'product', $('#id_product option:selected').val());
		if (!id_condition)
			return false;
		
		
		var html = '<tr id="'+id_condition+'"><td>{l s='Products'}</td><td>'+$('#id_product option:selected').html()+'</td><td><a href="#" onclick="delete_condition(\''+id_condition+'\');" class="btn btn-default"><i class="icon-remove"></i> {l s='Delete'}</a></td></tr>';
		appendConditionToGroup(html);
