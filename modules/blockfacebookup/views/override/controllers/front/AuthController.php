...
class AuthControllerCore extends FrontController
{
	...
	public function initContent()
	{
		...
		$tiposiva = Db::getInstance()->ExecuteS("SELECT ID_ERP,name FROM "._DB_PREFIX_."flexxus_equivalencia_tiposiva");
		
		$hook_ivas = '<div class="form-group">
					    <label>Condición de Iva</label>
					    <div class="row">
					        <div class="col-xs-12">
					            <div class="selector" id="uniform-tiposiva" style="padding:0px;">					                
					                <select id="tiposiva" name="tiposiva" class="form-control">
					                <option value="CF">Seleccione su condicion de iva </option>';

		foreach ($tiposiva as $iva){
			$hook_ivas .= '<option value="'.$iva['ID_ERP'].'">'.$iva['name'].'</option>';
		}
		...
		$this->context->smarty->assign(array(
				'HOOK_CREATE_ACCOUNT_FORM' => Hook::exec('displayCustomerAccountForm').$hook_ivas,
	...
	protected function processSubmitAccount()
	{
		...
		$CODIGOIVA = Tools::getValue('tiposiva');		
		...
		//$_POST['tiposiva'] = Tools::getValue('tiposiva', $CODIGOIVA);
			...
			if (!count($this->errors))
			{
				...
				if (!count($this->errors))
				{
					if ($customer->add())
					{
						...
						$id_prestashop = Db::getInstance()->getRow("SELECT MAX(id_customer) as customer FROM "._DB_PREFIX_."customer");
						$codigoIVA = Db::getInstance()->insert('flexxus_cliente_condicioniva',array(
								'CODIGOIVA' => pSQL(Tools::getValue('tiposiva')),
								'ID_ERP' => '',
								'id_prestashop' => $id_prestashop['customer']
						));
