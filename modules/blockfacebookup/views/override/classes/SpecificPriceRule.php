...
class SpecificPriceRuleCore extends ObjectModel
{
	...
	public function apply($products = false)
	{
		...
		foreach ($products as $product){
			//var_dump($products);
			SpecificPriceRule::applyRuleToProduct((int)$this->id, (int)$product['id_product'], (int)$product['id_product_attribute'],(int)$product['id_customer']);
		}
	}
	...
	public function getConditions()
	{
		...
		if ($conditions)
		{
			foreach ($conditions as &$condition)
			{
				...
				elseif ($condition['type'] == 'customer')
					$condition['id_customer'] = Db::getInstance()->getValue('SELECT id_customer
																								FROM '._DB_PREFIX_.'customer
																								WHERE id_customer='.(int)$condition['value']);
	...
	public function getAffectedProducts($products = false)
	{
			...
			foreach ($conditions_group as $id_condition_group => $condition_group)
			{
				...
				foreach ($condition_group as $condition)
				{
					...
					if ($condition['type'] == 'product'){
						$articulo = true;
						$id_product = $condition['value'];										
					}elseif ($condition['type'] == 'customer'){
						$customer = true;
						$id_customer = $condition['value'].' as id_customer';										
					}elseif ($condition['type'] == 'feature')
						$features = true;									
		...
		//$where .=' AND p.id_product='.$id_product;
		...
		if ($customer){
			...
			$query->leftJoin('customer', 'cs', 'cs.id_shop = p.id_shop_default');
		}else{
			$query->select('0 as id_customer');
			$query->leftJoin('customer', 'cs', 'cs.id_shop = p.id_shop_default');
		}
	...
	public static function applyRuleToProduct($id_rule, $id_product, $id_product_attribute = null,$id_customer)
	{
